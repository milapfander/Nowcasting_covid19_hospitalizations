...
### Interval correction with new quantiles:
# Function to calculate the PIT quantiles for one day and region
#' @param doa date of analysis
#' @param location region of nowcast
#' @param interval_vec vector of confidence niveaus
#' @param retrospective logical indicating retrospective analysis, default TRUE
interval_correction_pit <- function(doa, location,
                                    interval_vec = c(0.8, 0.95),
                                    retrospective = FALSE) {
  ...
  data_real$d <- as.numeric(gsub(pattern = "value_(\\.)?|d", x = data_real$d,
                                 replacement = ""))
  data_real <- data_real %>% mutate(d = d + 1) %>% filter(d <= 40) %>%
    group_by(date, age_group) %>% dplyr::summarise(realized = sum(value)) %>%
    arrange(desc(date))
  ...
  # until here almost the same as in Intervals.R
  
  # iterate over days 41-100 and calculate p0 values
  dat_p0 <- lapply(dates, function(date_calc){
    # load Bootstrap iterations
    dat_bs <- read_csv2(paste0("Nowcast_Hosp/03_Results/RKI_results", 
                               retro, date_calc, "/bootstrap_results_",
                               location, "_", date_calc, ".csv"))
    
    # join BS iterations and reported values
    dat_bs <- left_join(dat_bs,
                        data_today %>% select(date, age60, realized7),
                        by = c("base" = "date", "Altersgruppe" = "age60"))
    
    # calculate transformed quantile
    dat_bs <- dat_bs %>%
      rowwise() %>%
      mutate(p0 = ( rank(c_across(c(realized7, starts_with("Iteration"))))[[1]]
                    / (length(c_across(starts_with("Iteration"))) +1 )),
             date_calc = date_calc,
             horizon = as.numeric(date_calc - base)) %>%
      ungroup() %>%
      # only keep columns for identification and p0
      dplyr::select(date_calc, horizon, Altersgruppe, p0)
  })
  dat_p0 <- bind_rows(dat_p0)
  
  # compute quantiles of pit values
  quantiles <- sort(as.vector(vapply(interval_vec,
                                     function(x) {c((1 - x) / 2, 1 - (1 - x) / 2)},
                                     numeric(2))))
  dat_quantiles <- dat_p0 %>%
    group_by(horizon, Altersgruppe) %>%
    dplyr::summarise(quant_value = quantile(p0, probs = quantiles)) %>%
    ungroup() %>%
    mutate(quantile = paste0("quantile_",
                             rep(x = quantiles, times = nrow(.) / length(quantiles))))
  dat_quantiles <- dat_quantiles %>%
    pivot_wider(values_from = quant_value, names_from = quantile)
  
  # Determine new quantiles based on pit correction:
  dat_bs_today <- read_csv2(paste0("Nowcast_Hosp/03_Results/RKI_results", 
                                   retro, doa, "/bootstrap_results_",
                                   location, "_", doa, ".csv"))
  
  quantiles_new <- lapply(X = 1:nrow(dat_bs_today), FUN = function(x) {
    quantiles_row <- as.numeric(quantile(x = dat_bs_today[x, 3:ncol(dat_bs_today)],
                                         probs = dat_quantiles[x, 3:ncol(dat_quantiles)])) 
  })
  quantiles_new <- t(as.data.frame(quantiles_new))
  row.names(quantiles_new) <- NULL
  colnames(quantiles_new) <- paste0("nowcast7_", quantiles)
  
  # Replace current data with new estimated quantiles:
  data_today[1:nrow(quantiles_new), colnames(quantiles_new)] <-
    quantiles_new
  
  write.csv2(x = data_today, 
             file = paste0("Nowcast_Hosp/03_Results/RKI_results", 
                           retro, doa, 
                           "/pit_correction_nowcasting_results_", location, "_", doa, ".csv"),
             row.names = FALSE)
